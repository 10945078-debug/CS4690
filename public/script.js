...
$(() => {
  //const logRepo = new ApiLogRepository(); //All functions should be ajax calls
  const logRepo = new ApiLogRepository();

  //Function for loading courses dynamically
  async function LoadCourses(){ //Get rid of the promise parts 
    //const response = await axios.get<Course[]>('/api/v1/courses');
    //const data = response.data;
    const courses = logRepo.getCourses;
    //const data = response.data;
  ...
  async function loadLogs() {
    ...
    const logs = await logRepo.getLogs(courseId, uvuId);
    renderLogs(logs, uvuId);
  }

  function renderLogs(logs, uvuId) {
    $('#uvuIdDisplay').html(`Student Logs for ${uvuId}`);
    const items = logs.length > 0
      ? logs.map(log => `<li class="list-group-item">${log.text}</li>`).join('')
  ...
  async function addLog() {
    const courseId = $('#course').val;
    const uvuId = $('#uvuId').val;
    const text = $('#log_textarea').val;

    if (!text.trim()) return;

    try {
      //await logRepo.addLog({ courseId, uvuId, text});
      /*
...
*/
/*
async function LoadLogs(courseId: string): Promise<void> {
    ...
    const data = await logRepo.getLogs(courseId, $('#uvuId').val());
    console.log(data);

    $('#uvuIdDisplay').html(`Students Logs for ${$('#uvuId').val()}`);
    
    if(data && data.length > 0) {
      let items = data.map((log: Log) =>
